...
const main = async () => {
  ...
  const outPath = path.resolve(getString(args, 'out', 'data/generated/match-input.json'));
  const title = getString(args, 'title', 'Attacker vs Defender: Rollout Rumble');
  const attackerName = getString(args, 'attacker', 'Environment Attacker');
  const defenderName = getString(args, 'defender', 'Task Defender');
  ...
  const durationInSeconds = (compiled.totalFrames / 30).toFixed(2);
  console.log(`Wrote input props to ${outPath}`);
  console.log(`Events: ${events.length}`);
  console.log(`Rounds: ${compiled.rounds.length}`);
  console.log(`Estimated duration: ${durationInSeconds}s`);
};
